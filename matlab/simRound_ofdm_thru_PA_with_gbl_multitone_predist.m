function [mat_filename] = simRound_ofdm_thru_PA_with_gbl_multitone_predist(fc, nFFT, M_predist, K_predist, mat_filename)
...
sim_mat_filename = sprintf('gbl_multitone_M_%g_nFFT_%g.mat', M_predist, nFFT);
sim_mat_filename = strrep(sim_mat_filename, '+', '');
fullpath = [directory_name, '/', sim_mat_filename];

res = load(fullpath, 'h', 'P');
h = res.h;
