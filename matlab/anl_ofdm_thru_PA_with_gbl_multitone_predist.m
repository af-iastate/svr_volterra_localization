...
for fc=[fcs(1)]
    ...
    clf
    subplot(211)
    h = plot(M_predists, m2db(avg_test_evms));
    hold on
    scatter(M_predists(~test_matches), m2db(avg_test_evms(~test_matches)), 'CData', h.Color, ...
        'SizeData', 25, 'MarkerFaceColor', 'White');
    scatter(M_predists(test_matches), m2db(avg_test_evms(test_matches)), 'Cdata', h.Color, ...
        'marker', 'o', 'MarkerFaceColor', h.Color, 'MarkerEdgeColor', 'none', 'SizeData', 25)
    hold off
    title('Volt Gbl Predist. Avg. EVM')
    xlabel('M_{pred}')
    ylabel('dB')
    xticks(M_predists)
    ...
    scatter(M_predists(test_matches), m2db(worst_test_evms(test_matches)), 'Cdata', h.Color, ...
        'marker', 'o', 'MarkerFaceColor', h.Color, 'MarkerEdgeColor', 'none', 'SizeData', 25)
    hold off
    title('Volt Gbl Predist. Worst EVM')
    xlabel('M_{pred}')
    ylabel('dB')
    xticks(M_predists)
