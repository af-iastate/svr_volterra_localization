...
directory_name = 'D:/simulations/rls_lcl_post_ofdm_sawp';
    ...
    for fc_train=fcs
        for nFFT=[nFFTs(2)]
            for M_predist=M_predists
                ...
                fprintf('--- NEW SIM:  fc_test=%g  fc_train=%g  nFFT=%g  M=%g  K=%g ---\n\n', fc_test, fc_train, nFFT, M_predist, K_predist)
                if ~exist(fullpath, 'file')
                    close all;
                    simRound_swap_ofdm_thru_PA_with_rls_post_lcl_predist(fc_test, fc_train, nFFT, M_predist, K_predist, fullpath)
                    drawnow
                    pause(1)
                end
            end
