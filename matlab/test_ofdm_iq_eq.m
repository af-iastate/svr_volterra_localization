...
tx_symbols = [0, 0, randi([0, qam_nSymbols-1], ofdm_nFFT - 4, 1).', 0, 0].';
...
tx_output = tx_iqUp;
tx_power = norm(tx_output);

% --- Transmission Line Distort ---
rx_input = tx_output;
rx_input = awgn(rx_input, 40) * 1.3;  % Add noise

% --- Receiver Power Equalize ---
rx_inputEq = (tx_power/norm(rx_input)).*rx_input;

% --- Receiver I/Q Demodulate ---
rx_tEnd = ofdm_TsUp * (length(rx_inputEq)-1);
rx_t = (0:ofdm_TsUp:rx_tEnd).';
[rx_iqFiltB, rx_iqFiltA] = butter(5, (2*ofdm_scs*ofdm_nFFT/2)/(ofdm_FsUp/2), 'low');
% i/q
rx_i = 2*filtfilt(rx_iqFiltB, rx_iqFiltA, rx_inputEq .* cos(2*pi*iq_fLo.*rx_t));
rx_q = 2*filtfilt(rx_iqFiltB, rx_iqFiltA, rx_inputEq .* -sin(2*pi*iq_fLo.*rx_t));
rx_iq = rx_i + 1j.*rx_q;

% --- Receiver Remove Offset ---
rx_idxA = ofdm_nCycPreUp+1;
...
hold on
plot(rx_t(rx_idxA : end), real(rx_sync))
...
plot(rx_t(rx_idxA : end), imag(rx_sync))
