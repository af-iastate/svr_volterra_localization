function [mat_filename] = simRound_swap_ofdm_thru_PA_with_rls_lcl_predist(fc_test, fc_train, upsample_amnt, nFFT, M_predist, K_predist, mat_filename)
...
% --- Init Volterra RLS ---
rls_factor = upfactor / upsample_amnt;
% K_predist = 3;
...
sim_mat_filename = sprintf('rls_lcl_ofdm_fc_%g_nFFT_%g_up_%d_M_%g_K_%g.mat', fc_train, nFFT, upsample_amnt, M_predist, K_predist);
sim_mat_filename = strrep(sim_mat_filename, '+', '');
fullpath = [directory_name, '/', sim_mat_filename];
res = load(fullpath, 'h_i', 'h_q');
h_i = res.h_i;
h_q = res.h_q;
clearvars res;
