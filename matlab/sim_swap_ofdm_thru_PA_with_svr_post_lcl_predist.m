    ...
    for fc_train=fcs
        for nFFT=[nFFTs(2)]
            for M_svr=M_svrs
                ...
                fprintf('--- NEW SIM:  fc_test=%g  fc_train=%g  nFFT=%g  M=%g ---\n\n', fc_test, fc_train, nFFT, M_svr)
                if ~exist(fullpath, 'file')
                    close all;
                    simRound_swap_ofdm_thru_PA_with_svr_post_lcl_predist(fc_test, fc_train, nFFT, M_svr, fullpath)
                    drawnow
                    pause(3)
                end
            end
