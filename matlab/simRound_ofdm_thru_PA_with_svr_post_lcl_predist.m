function [mat_filename] = simRound_ofdm_thru_PA_with_svr_post_lcl_predist(fc, nFFT, M_svr, mat_filename)
...
offset = 1 + nCycPre;
nSamples = nFFT;
Y_train = toeplitz(y_train, [y_train(1), zeros(1, M_svr-1)]);
h_Mdl = fitrsvm(Y_train, x_train, svr_options{:});


% --- ReSimulate Training Data with Predist ---
X_train = toeplitz(x_train, [x_train(1), zeros(1, M_svr-1)]);
u_train = h_Mdl.predict(X_train);
