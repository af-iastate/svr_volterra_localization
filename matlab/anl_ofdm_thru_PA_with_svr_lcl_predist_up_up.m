...
for M_svr=M_svrs(6)
    for jj=1:length(fcs)
        ...
        hold off
        my_loc = 'north';
        if any([m2db(worst_train_evms(5:7)); m2db(worst_test_evms(5:7))] >= -2)
            my_loc = 'south';
        end
...
%     gif_path = sprintf('gif/svr_lcl_predist_up_up/evms.gif');
...
%     axis(1.1.*[-1, 1, -1, 1])
%     pbaspect([1, 1, 1])
%     title(sprintf('Const fc=%d MHz, nFFT=%g, up=%03d, M_{svr}=%g  train_{mt}=%d, test_{mt}=%d', ...
%          round(fc/1e6), nFFT, upsample_amnt, M_svr, train_match, test_match))
%     box on;
%     legend({'TX', 'RX_{train}', 'RX_{test}'});

    
%     gif_path = sprintf('gif/svr_lcl_predist_up_up/fc_%d_m_%d.gif', round(fc/1e6), M_svr);
