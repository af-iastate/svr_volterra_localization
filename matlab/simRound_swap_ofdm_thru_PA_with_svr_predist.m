function [mat_filename] = simRound_swap_ofdm_thru_PA_with_svr_predist(fc_test, fc_train, upsample_amnt, nFFT, M_svr, mat_filename)
close all;
...
sim_mat_filename = sprintf('svr_res_fc_%g_nFFT_%g_up_%d_Msvr_%g.mat', fc_train, nFFT, upsample_amnt, M_svr);
sim_mat_filename = strrep(sim_mat_filename, '+', '');
fullpath = [directory_name, '/', sim_mat_filename];
res = load(fullpath, 'I_Mdl', 'Q_Mdl');
I_Mdl = res.I_Mdl;
Q_Mdl = res.Q_Mdl;
clearvars res;
...
% y_train_iq = iq_demodulate(y_train, Fs, fc_test, (headroom_coeff*scs*nFFT/2));
...
% u_train = iq_modulate(u_train_iq, Fs, fc_test);
% y_train_hat = forward_f(u_train);
% y_train_hat_iq = iq_demodulate(y_train_hat, Fs, fc_test, (headroom_coeff*scs*nFFT/2));
% 
% 
...
u_test = iq_modulate(u_test_iq, Fs, fc_test);
