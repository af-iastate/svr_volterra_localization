...
fcs = [19e6, 27e6, 35e6];
nFFT = 128;
M_svrs = 2*[1, 2, 4, 8, 10, 15, 20, 30, 40];
upsample_amnt = 2;
...
for fc=[fcs(2)]
    ...
    % --- Train ---
    h_train = plot(M_svrs, m2db(avg_train_evms), 'color', C(1,:));
    scatter(M_svrs(~train_matches), m2db(avg_train_evms(~train_matches)), 'CData', h_train.Color, ...
        'SizeData', 25, 'MarkerFaceColor', 'White');
    scatter(M_svrs(train_matches), m2db(avg_train_evms(train_matches)), 'Cdata', h_train.Color, ...
        'marker', 'o', 'MarkerFaceColor', h_train.Color, 'MarkerEdgeColor', 'none', 'SizeData', 25)

    % --- Test ---
    h_test = plot(M_svrs, m2db(avg_test_evms), 'color', C(2,:));
    scatter(M_svrs(~test_matches), m2db(avg_test_evms(~test_matches)), 'CData', h_test.Color, ...
        'SizeData', 25, 'MarkerFaceColor', 'White');
    scatter(M_svrs(test_matches), m2db(avg_test_evms(test_matches)), 'Cdata', h_test.Color, ...
    ...
    xlabel('$M_{pred}$', 'Interpreter', 'latex')
