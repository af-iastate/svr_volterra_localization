...
%         title(sprintf('Worse EVM $M_\\mathrm{pred}=%d$, $u_d=%d$', M_svr, upsample_amnt), 'Interpreter', 'Latex')
%         for ii = 1:length(b_h)
%             b_hh = b_h(ii);
%             b_hh.FaceAlpha = '0.3';
%         end
        
%         gif_path = sprintf('gif/svr_lcl_swap_m_up/up_%d.gif', upsample_amnt);
%         exportgraphics(gca, gif_path, "Append", true)
    end 
    pause
end

function varargout = get_sim(fc_test, fc_train, nFFT, up_amnt, M_svr, varargin)
    directory_name = 'simulations/svr_swap';
    mat_filename = sprintf('svr_res_fc_test_%g_fc_train_%g_nFFT_%g_up_%d_Msvr_%g.mat', fc_test, fc_train, nFFT, up_amnt, M_svr);
