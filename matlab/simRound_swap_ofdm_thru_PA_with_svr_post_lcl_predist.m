function [mat_filename] = simRound_swap_ofdm_thru_PA_with_svr_post_lcl_predist(fc_test, fc_train, nFFT, M_svr, mat_filename)
...
sim_mat_filename = sprintf('svr_lcl_post_ofdm_fc_%g_nFFT_%g_Msvr_%g.mat', fc_train, nFFT, M_svr);
sim_mat_filename = strrep(sim_mat_filename, '+', '');
fullpath = [directory_name, '/', sim_mat_filename];
res = load(fullpath, 'h_Mdl');
h_Mdl = res.h_Mdl;
clearvars res;
...
test_EVMs = (abs(tx_test_grid - rx_test_grid)) ./ abs(tx_test_grid);
